...
class CUI_Cuts
  ...
  def Image_Cuts # Image Cutter
    ct_x = 1
    # y
    (1..@Y_Size.to_i).each do |ct_y, y_pos|
      puts "ct_x:#{ct_x}"
      puts "ct_y:#{ct_y}"

      x_pos = ct_y.to_i == 1 ? @Crop_X.to_i + ((ct_y.to_i - 1) * 16) : @Crop_X.to_i + ((ct_y.to_i - 1) * 16) + ((ct_y.to_i - 1) * 16)
      y_pos = if ct_y.to_i == 1
                @Crop_Y
              else
                @Crop_Y.to_i - ((ct_y.to_i - 1) * 16)
              end

      print "----\n"
      # x
      (1..X_Size.to_i).each do |ct_x|
        print 'X: '
        puts ct_x.to_i
        print 'Y: '
        puts ct_y.to_i
        ...
        # override
        c_Image = @image.crop(Magick::SouthEastGravity, x_pos.to_i, y_pos.to_i, 64, 64, true) do
  ...
  def Image_Write
    exb = Magick::Image.new(X_Size.to_i * 64, Y_Size.to_i * 64) do
